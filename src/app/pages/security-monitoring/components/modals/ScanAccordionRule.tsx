...
const ScanAccordionRule = ({ data }: any) => {
  const { mode } = useRecoilValue(modeAtomsAtom);
  const [isOpen, setIsOpen] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [ticketOpen, setTicketOpen] = useState(false);
  const [isChecked, setIsChecked] = useState(false);
  const [checkPos, setCheckPos] = useState(0);
  const [mess, setMess] = useState("")

  return (
    <div className="w-full">
      <button
        className={`grid grid-cols-7 gap-[8px] border-bottom h-[52px] place-content-center ${
        ...
        <p
          className={
            data?.severity?.toUpperCase() === "HIGH"
              ? "text-[#FF7D30] text-[12px] font-semibold uppercase"
              : "text-[#2AB849] text-[12px] font-semibold uppercase"
          }
        >
          {data?.severity ? data?.severity : "N/A"}
        </p>
        <div className="text-[12px] flex items-center justify-center flex-col gap-1">
          <div className="w-full rounded-full h-3 bg-[#DADADA] overflow-hidden">
            <span
              className={`${
                data?.percentage < 100 ? "bg-[#FF161A]" : "bg-[#2AB849]"
              } h-3 block`}
              style={{ width: `${Math.round(data?.percentage)}%` }}
            ></span>
          </div>
          <span className="font-medium flex items-center justify-between w-full">
            <p>{`${data?.passed_checks_count} / ${data?.total_checks_count}`}</p>
            <p>{`${Math.round(data?.percentage)}%`}</p>
          </span>
        </div>
      ...
      {isOpen &&
        data.checks.map((check: any, idx: number) => (
          <div
            className={`w-full font-medium md:pl-8 p-10 ${
              mode === "dark" ? "bg-lightDark" : "bg-white"
            } ${idx !== data?.checks.length - 1 ? "border-bottom-2" : ""}`}
            key={check?.rule_code}
          >
            <div className="grid grid-cols-3 gap-y-4">
              ...
              <p
                className={
                  check?.status_code.toLowerCase().includes("fail")
                    ? "text-[#FF161A] col-span-2"
                    : "text-[#2AB849] col-span-2"
                }
              >
                {check?.status_code ? check?.status_code : "N/A"}
              </p>
              <p className="font-semibold">Rule Code:</p>
              <p className="text-left col-span-2">{check?.rule_code ? check?.rule_code : "N/A"}</p>
              <p className="font-semibold">Provider:</p>
              <p className="text-left col-span-2">{check?.provider ? check?.provider : "N/A"}</p>
              <p className="font-semibold">Message:</p>
              <p className="text-left col-span-2">{check?.status_detail ? check?.status_detail : "N/A"}</p>
              <p className="font-semibold">Region:</p>
              <p className="text-left col-span-2">{check?.region ? check?.region : "N/A"}</p>
              <p className="font-semibold">Description:</p>
              <p className="text-left col-span-2">{check?.remediation_desc ? check?.remediation_desc : "N/A"}</p>
            </div>
            <button
              onClick={() => {
                if(check?.status_code.toLowerCase().includes("fail")) {
                    setIsModalOpen(true);
                    setCheckPos(idx);

                } else {
                    setMess("Nothing to resolve.All checks passed");
                }
                // setIsOpen(false);
                // setIsChecked(false);
              }}
            ...
            {
                mess && (<p className="font-semibold text-[#2AB849] text-[14px]">{mess}</p>)
            }
          </div>
        ))}
      </div>
          ...
          >
            ...
            <div className="w-full">
              ...
              <div className="mt-4 font-medium grid md:grid-cols-3 gap-3 pb-2 border-bottom px-4">
                ...
                <p
                  className={`${
                    data?.checks[checkPos]?.status_code.toLowerCase().includes("fail")
                      ? "text-[#FF161A]"
                      : "text-[#2AB849]"
                  } md:col-span-2`}
                >
                  {data?.checks[checkPos]?.status_code ? data?.checks[checkPos]?.status_code : "N/A"}
                </p>
                {/* <p>Resource Id:</p>
              <p className="md:col-span-2">sg-04cc9e5ccd9ca7f80</p> */}
                <p className="font-semibold">Prvider:</p>
                <p className="md:col-span-2">
                  {data?.checks[checkPos]?.provider ? data?.checks[checkPos]?.provider : "N/A"}
                </p>
                <p className="font-semibold">Message:</p>
                <p className="md:col-span-2">
                  {data?.checks[checkPos]?.status_detail ? data?.checks[checkPos]?.status_detail : "N/A"}
                </p>
                <p className="font-semibold">Description:</p>
                <p className="md:col-span-2">
                  {data?.checks[checkPos]?.finding_desc ? data?.checks[checkPos]?.finding_desc : "N/A"}
                </p>
              </div>
              <div className="mt-4 px-4 pb-2">
                ...
                <div className="p-4 font-medium rounded-md bg-[#284CB31A] flex items-center gap-2">
                  ...
                  <p className="w-[90%]">
                    {data?.checks[checkPos]?.remediation_desc ? data?.checks[checkPos]?.remediation_desc : "N/A"}
                  </p>
                </div>
              </div>
            </div>
...
export default ScanAccordionRule;
